Java.perform(() => {
    ...
    var HttpRequestClass = Java.use('com.android.volley.toolbox.BaseHttpStack');
    ...
    HttpRequestClass.executeRequest.overload('com.android.volley.Request', 'java.util.Map')
        .implementation = function (request, headers) {
        send('Inside of executeRequest new implementation');
        
        send('Request: ' + request);
        send('Headers: ');
        
        var keySet = headers.keySet().toArray();
        for (var i = 0; i < keySet.length; i++) {
            var headerKey = keySet[i];
            var headerValue = headers.get(headerKey).toString();
            send(headerKey + ': ' + headerValue);
        }

        var retExec = this.executeRequest(request, headers);
        return retExec;
    };
  });